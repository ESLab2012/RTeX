...
public:
  ...
  void clear_handles_timers() override
  {
    timer_handles_.clear();
  }
...
//#endif
  ...
  void remove_null_handles_lf(rcl_wait_set_t * wait_set) override
  {
    for (size_t i = 0; i < timer_handles_.size(); ++i) {
      if (!wait_set->timers[i]) {
        timer_handles_[i].reset();
  ...
  bool collect_entities_lf(const WeakCallbackGroupsToNodesMap & weak_groups_to_nodes) override
  {
    bool has_invalid_weak_groups_or_nodes = false;
    for (const auto & pair : weak_groups_to_nodes) {
      auto group = pair.first.lock();
  ...
  bool add_handles_to_wait_set_lf(rcl_wait_set_t * wait_set) override
  {
    for (const std::shared_ptr<const rcl_timer_t> & timer : timer_handles_) {
      if (rcl_wait_set_add_timer(wait_set, timer.get(), NULL) != RCL_RET_OK) {
        RCUTILS_LOG_ERROR_NAMED(
  ...
  static void
  lock_free_get_next_timer(
    uintptr_t & pointer_value,
    rclcpp::AnyExecutable & any_exec,
    const WeakCallbackGroupsToNodesMap & weak_groups_to_nodes)
  {
    std::shared_ptr<TimerBase> & timer = reinterpret_cast<rclcpp::TimerBase::SharedPtr&>(pointer_value);
    // Find the group for this handle and see if it can be serviced
    auto group = get_group_by_timer(timer, weak_groups_to_nodes);
    if (!group->can_be_taken_from().load()) {
      printf("ERROR!!!!\r\n");
  ...
  static void
  lock_free_get_next_sub(
    uintptr_t & pointer_value,
    rclcpp::AnyExecutable & any_exec,
    const WeakCallbackGroupsToNodesMap & weak_groups_to_nodes)
  {
    std::shared_ptr<SubscriptionBase> & subscription = reinterpret_cast<rclcpp::SubscriptionBase::SharedPtr&>(pointer_value);
    auto group = get_group_by_subscription(subscription, weak_groups_to_nodes);
    if (!group->can_be_taken_from().load()) {
      printf("ERROR!!!!\r\n");
      return;
  ...
  static void
  lock_free_get_next_service(
    uintptr_t & pointer_value,
    rclcpp::AnyExecutable & any_exec,
    const WeakCallbackGroupsToNodesMap & weak_groups_to_nodes)
  {
    std::shared_ptr<ServiceBase> & service = reinterpret_cast<rclcpp::ServiceBase::SharedPtr&>(pointer_value);
    auto group = get_group_by_service(service, weak_groups_to_nodes);
    if (!group->can_be_taken_from().load()) {
      printf("ERROR!!!!\r\n");
      return;
  ...
  static void
  lock_free_get_next_client(
    uintptr_t & pointer_value,
    rclcpp::AnyExecutable & any_exec,
    const WeakCallbackGroupsToNodesMap & weak_groups_to_nodes)
  {
    std::shared_ptr<ClientBase> & client = reinterpret_cast<rclcpp::ClientBase::SharedPtr&>(pointer_value);
    auto group = get_group_by_client(client, weak_groups_to_nodes);
    if (!group->can_be_taken_from().load()) {
      printf("ERROR!!!!\r\n");
      return;
  ...
  static void
  lock_free_get_next_waitable(
    uintptr_t & pointer_value,
    rclcpp::AnyExecutable & any_exec,
    const WeakCallbackGroupsToNodesMap & weak_groups_to_nodes)
  {
    std::shared_ptr<Waitable> & waitable = reinterpret_cast<rclcpp::Waitable::SharedPtr&>(pointer_value);
    auto group = get_group_by_waitable(waitable, weak_groups_to_nodes);
    if (!group->can_be_taken_from().load()) {
      printf("ERROR!!!!\r\n");
      return;
  ...
  {
    rclcpp::Waitable::SharedPtr waitable_test;
    //rclcpp::TimerBase::SharedPtr timer_test;
    std::thread::id thread_id = std::this_thread::get_id();
    uintptr_t temp_value = lockFree_deleteMin((*(uint32_t*)&thread_id), type);
    if (*type == 1) 
    { 
      lock_free_get_next_timer(temp_value, any_exec, weak_groups_to_nodes);
    } 
    else if (*type == 2)
    {
      lock_free_get_next_sub(temp_value, any_exec, weak_groups_to_nodes);
    } 
    else if (*type == 3)
    {
      lock_free_get_next_service(temp_value, any_exec, weak_groups_to_nodes);
    } 
    else if (*type == 4)
    {
      lock_free_get_next_client(temp_value, any_exec, weak_groups_to_nodes);
    } 
    else if (*type == 5)
    {
      lock_free_get_next_waitable(temp_value, any_exec, weak_groups_to_nodes);
    }
    return;
  }
  ...
  size_t timer_handles_num() const override
  {
    return timer_handles_.size();
  }

  size_t waitable_handles_num() const override
  {
    return waitable_handles_.size();
  }
