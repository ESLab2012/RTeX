...
Executor::get_next_ready_executable(AnyExecutable & any_executable)
{
  ...
  success = get_next_ready_executable_from_map(any_executable, weak_groups_to_nodes_);
  return success;
}
  ...
  weak_groups_to_nodes)
{
  ...
  if (callback_priority_enabled) {
    ...
    return get_next_ready_executable_from_map(any_executable, weak_groups_to_nodes_);
  }
...
bool
Executor::RTeX_get_next_ready_executable(AnyExecutable & any_executable)
{
  bool success = RTeX_get_next_ready_executable_from_map(any_executable, weak_groups_to_nodes_);
  return success;
}

bool
Executor::RTeX_get_next_ready_executable_from_map(
  AnyExecutable & any_executable,
  const rclcpp::memory_strategy::MemoryStrategy::WeakCallbackGroupsToNodesMap &
  weak_groups_to_nodes)
{
  ...
  memory_strategy_->get_next_callback(any_executable, weak_groups_to_nodes, &type);
  switch (type) {
    case 0://EMPTY
      return success;
    case 1://timer
    ...
    case 4://client
      success = true;
      break;
    case 5://waitable, intra-process use this type in ROS 2.
    {  
      any_executable.data = any_executable.waitable->take_data();
      success = true;
      break;
    ...
    default://others
      printf("[ERROR] in RTeX_get_next_ready_executable_from_map !\r\n");
      break;
  }
  // At this point any_executable should be valid with either a valid subscription
  // or a valid timer, or it should be a null shared_ptr
...
bool
Executor::RTeX_get_next_executable(AnyExecutable & any_executable, std::chrono::nanoseconds timeout)
{
  bool success = false;
  (void) timeout;

  success = RTeX_get_next_ready_executable(any_executable);
  if (!success) {
    RTeX_wait_for_work();
    if (!spinning.load()) {
      return false;
    ...
    // Try again in case we have expired timers
    success = RTeX_get_next_ready_executable(any_executable);
  }
  return success;
}
...
void
Executor::RTeX_wait_for_work()
{
  std::thread::id thread_id = std::this_thread::get_id();
  // get a local copy of `ptr_global_timerList`
  callback_map* ptr_temp_timerList = ptr_global_timerList.load(); 
  // get the next timeout in timerList
  int64_t first_timeout = ptr_temp_timerList[0].next_call_time;
  // get first timer's ptr in timerList   
  uintptr_t first_timer = ptr_temp_timerList[0].callback;         
  // flag to show if there are any expired timers
  bool is_ready = false;    
  // time to the next expired timer
  int64_t time_to_wait = 0; 
  
  std::shared_ptr<rclcpp::TimerBase> & timer = reinterpret_cast<rclcpp::TimerBase::SharedPtr&>(first_timer);
  ...
  is_ready = rcl_timerList_check_timer_expired(&rcltimer, &time_to_wait, &first_timeout);

  // If the first timer is not expired (is_ready==false), then we should wait for some time!
  if (!is_ready) {
    struct timespec ts;
    pthread_mutex_lock(&MDList_mutex);
    clock_gettime(CLOCK_REALTIME, &ts);
    ts.tv_nsec += time_to_wait;
    if (ts.tv_nsec > 1000000000) {
      ts.tv_sec += 1;
      ts.tv_nsec -= 1000000000;
    }
    pthread_cond_timedwait(&MDList_not_empty, &MDList_mutex, &ts);
    pthread_mutex_unlock(&MDList_mutex);

    // Update flag `is_ready` again, since we may be waken up due to some subs get ready!
    is_ready = rcl_timerList_check_timer_expired(&rcltimer, &time_to_wait, &first_timeout);
  }

  // If indeed some timers have been expired (is_ready==true)!
  int64_t temp_timeout, temp_time_to_wait, period;
  uintptr_t temp_timer;
  bool have_map_expired_timers = false; 
  if (is_ready) {
    vector<uintptr_t> expired_timers;     
    callback_map* ptr_local_timerList = TID_to_MAP[(*(uint32_t*)&thread_id)]; 
    for (int i=0; i<Timer_num; ++i) {
      *(ptr_local_timerList+i) = *(ptr_temp_timerList+i);
    }
    for (int i=0; i<Timer_num; ++i) {
      temp_timer = (ptr_local_timerList+i)->callback;
      temp_timeout = (ptr_local_timerList+i)->next_call_time;
      std::shared_ptr<rclcpp::TimerBase> & ptr_temp_timer = reinterpret_cast<rclcpp::TimerBase::SharedPtr&>(temp_timer);
      auto ptr_timer_handle = ptr_temp_timer->get_timer_handle();
      rcltimer.impl = ptr_timer_handle->impl;
      is_ready = rcl_timerList_check_timer_expired(&rcltimer, &temp_time_to_wait, &temp_timeout);
      if (is_ready) {
        expired_timers.emplace_back(temp_timer);
        have_map_expired_timers = true;
        if(rcl_timer_get_period(&rcltimer,&period)!=RCL_RET_OK)
          printf("[ERROR] in rcl_timer_get_period !\n");
        if(lock_free_call(&rcltimer, &temp_timeout, period)!=RCL_RET_OK)
          printf("[ERROR] in lock_free_call !\n");
        (ptr_local_timerList+i)->next_call_time = temp_timeout;
        continue;
      }
      break;
    }

    if(have_map_expired_timers){
      sort(ptr_local_timerList,ptr_local_timerList+Timer_num,[](callback_map a,callback_map b){
        return a.next_call_time < b.next_call_time;
      });
      if (ptr_global_timerList.compare_exchange_strong(ptr_temp_timerList, ptr_local_timerList)) {
        TID_to_MAP[(*(uint32_t*)&thread_id)] = ptr_temp_timerList;
        for (auto iter=expired_timers.begin(); iter!=expired_timers.end(); ++iter) {
          if (readyList.insert(*iter,(*(uint32_t*)&thread_id))==false) {
            printf("[LOG] Timer(s) %ld have inserted by other thread !\n",*iter);
          }
        }
      }
    }
...
void
Executor::RTeX_check_and_update_timerList()
{
  std::thread::id thread_id = std::this_thread::get_id();
  // get a local copy of `ptr_global_timerList`
  callback_map* ptr_temp_timerList = ptr_global_timerList.load(); 
  ...
  // flag to show if there are any expired timers.
  bool is_ready = false; 
  // time margin to the next expired timer.   
  int64_t time_to_wait = 0; 
  ...
  // If the first timer is not expired (is_ready==false), then we can safely wait for some time
  if (!is_ready) {
    return;
  }
...
bool
Executor::RTeX_execute_any_executable(AnyExecutable & any_exec)
{
  if (!spinning.load()) {
    return false;
  }
  ...
  // check the timer list in case some timers has expired
  RTeX_check_and_update_timerList();

  // Reset the callback_group, regardless of type
  any_exec.callback_group->can_be_taken_from().store(true);
  // Wake the wait, because it may need to be recalculated or work that was previously blocked is now available.
  try {
    interrupt_guard_condition_.trigger();
  } catch (const rclcpp::exceptions::RCLError & ex) {
    throw std::runtime_error(
