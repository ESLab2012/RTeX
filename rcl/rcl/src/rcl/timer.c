...
rcl_ret_t
rcl_timer_call_lockFree(rcl_timer_t * timer)
{
  if (rcutils_atomic_load_bool(&timer->impl->canceled)) {
    RCL_SET_ERROR_MSG("timer is canceled");
    return RCL_RET_TIMER_CANCELED;
...
bool
rcl_timerList_check_timer_expired(const rcl_timer_t * timer, int64_t * time_until_next_call, int64_t* next_call_time)
{
  bool ret_val = false;
  if (rcutils_atomic_load_bool(&timer->impl->canceled)) {
    return RCL_RET_TIMER_CANCELED;
  ...
  *time_until_next_call = *next_call_time - now;
...
rcl_ret_t
lock_free_call(const rcl_timer_t * timer, int64_t *next_call_time, int64_t period){
  rcl_time_point_value_t now;
  rcl_ret_t now_ret = rcl_clock_get_now(timer->impl->clock, &now);
  if (now_ret != RCL_RET_OK) {
    return now_ret;  // rcl error state should already be set.
...
rcl_ret_t
rcl_timer_get_last_call_time(rcl_timer_t * timer, int64_t * last_call_time)
{
  RCL_CHECK_ARGUMENT_FOR_NULL(timer, RCL_RET_INVALID_ARGUMENT);
  RCL_CHECK_ARGUMENT_FOR_NULL(timer->impl, RCL_RET_TIMER_INVALID);
  RCL_CHECK_ARGUMENT_FOR_NULL(last_call_time, RCL_RET_INVALID_ARGUMENT);
  *last_call_time = rcutils_atomic_load_uint64_t((atomic_uint_least64_t *)(&timer->impl->last_call_time));
  return RCL_RET_OK;
}

rcl_ret_t
rcl_timer_get_next_call_time(rcl_timer_t * timer, int64_t * next_call_time)
{
  RCL_CHECK_ARGUMENT_FOR_NULL(timer, RCL_RET_INVALID_ARGUMENT);
  RCL_CHECK_ARGUMENT_FOR_NULL(timer->impl, RCL_RET_TIMER_INVALID);
  RCL_CHECK_ARGUMENT_FOR_NULL(next_call_time, RCL_RET_INVALID_ARGUMENT);
  *next_call_time = rcutils_atomic_load_uint64_t((atomic_uint_least64_t *)(&timer->impl->next_call_time));
  return RCL_RET_OK;
}

void
rcl_store_next_call_time(rcl_timer_t * timer, uint64_t next_call_time)
{
  rcutils_atomic_store(&timer->impl->next_call_time, next_call_time);
}
